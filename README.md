...
- [`docs/postman/`](./docs/postman) — Postman collection (74 requests, 9 folders) + environment for manual API exploration

## HOPLA convention — Postman collections

Every `@hopla/*` SDK package ships a Postman collection at `docs/postman/`, built from the
package's catalog (no hand-edits). Drop the JSON files into the **HOPLA Tools** team workspace in
Postman so all `@hopla/*` collections live side-by-side and the team can fork or run them
without setup.

For this package, the collection is regenerated by:

```bash
npm run postman:generate
```

The script (`scripts/generate-postman.ts`) reads `src/services.ts` as the single source of truth
and emits a fresh `docs/postman/iFreeiCloud.postman_collection.json` with one folder per service
category. Run it after editing the catalog and commit both the script-generated JSON and the
catalog change in the same commit.
